...
export default function BusinessDetail() {
  ...
  useEffect(() => {
    ...
    if (businessUserId) {
      fetch(
        `https://influencerhub1-g8dshgbwhgb9djfd.southeastasia-01.azurewebsites.net/api/business/get-business-by-user-id/${businessUserId}`
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const res = await fetch(
          `https://influencerhub1-g8dshgbwhgb9djfd.southeastasia-01.azurewebsites.net/api/review/review-of-business/${viewerId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
